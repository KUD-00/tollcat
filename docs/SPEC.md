...
仪表主角数字是**从量**（这个月云在烧多少）。本月订阅永远单独一行，不藏进筛选里。Widget 在「算进固定订阅」打开时仍用含订阅的合计——锁屏只有一个数字，漏掉 Max 就是错的。筛选关掉订阅之后，那一行改成「未计入」，Widget 和分享的合计也不再含它。
...
- **仅从量**（默认）：等于以前的行为。订阅那一行都在，写「未计入」。
- **含订阅**：大数字 = 从量 + 本月订阅（`totalUSD`），预计月底、
  构成、对比、趋势、分享卡、Widget 全部同口径，订阅那行写「已计入」。

**订阅口径不算「筛选」**：它不点亮工具栏的筛选图标、不进限定语
（`DashboardFilter.isActive` 只看时间和排除名单）——订阅行的
「已计入 / 未计入」就是它的限定语。「即将扣款」模块也不再跟着它隐藏：
口径管的是合计怎么算，那张卡说的是几天后要扣钱。
