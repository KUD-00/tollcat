...
/// Oxahost 客户发票（`GET /api/invoice`）。
///
/// 文档：https://my.oxahost.com/userapi/
...
public struct OxahostBillingProvider: BillingProvider, Sendable {
    public static var descriptor: ProviderDescriptor { ProviderCatalog.oxahost }
    public static let apiHost = "my.oxahost.com"
    ...
    public func fetch(
        ...
        for inv in invoices {
            ...
            try currencies.observe((currency?.isEmpty == false) ? currency! : "USD", providerID: .oxahost)
    ...
    private func authHeaders(credential: Credential) throws -> [String: String] {
        let email = try? RequiredCredential.value(.email, in: credential, providerID: .oxahost)
        let clientID = try? RequiredCredential.value(.clientID, in: credential, providerID: .oxahost)
        let secret = (try? RequiredCredential.value(.clientSecret, in: credential, providerID: .oxahost))
            ?? (try? RequiredCredential.value(.apiToken, in: credential, providerID: .oxahost))
            ?? (try? RequiredCredential.value(.apiKey, in: credential, providerID: .oxahost))
        ...
        let token = try RequiredCredential.value(.apiToken, in: credential, providerID: .oxahost)
        return [
    ...
    private func loadInvoices(headers: [String: String]) async throws -> [Invoice] {
        ...
        if let list = try? ProviderHTTP.decode([Invoice].self, from: data, providerID: .oxahost) {
            return list
        }
        let envelope = try ProviderHTTP.decode(InvoiceList.self, from: data, providerID: .oxahost)
