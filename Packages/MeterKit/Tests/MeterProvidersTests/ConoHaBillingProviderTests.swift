...
struct ConoHaBillingProviderTests {
    private let calendar = LiveProviderHarness.calendar
    private let now = LiveProviderHarness.now
    private let username = "conoha-user-MUST-NOT-LEAK"
    private let password = "conoha-pass-MUST-NOT-LEAK"
    private let tenant = "tenant-MUST-NOT-LEAK"
    private let token = "conoha-token-MUST-NOT-LEAK"
    ...
    func sumsInvoices() async throws {
        ...
        let authURL = ConoHaBillingProvider.identityURL(region: "tyo1", path: "/v2.0/tokens")
        let listURL = ProviderURL.https(
            host: "account.tyo1.conoha.io",
    ...
    private func provider(_ client: any HTTPClient) -> ConoHaBillingProvider {
        ConoHaBillingProvider(
            httpClient: client,
            now: { now },
            calendar: calendar,
