...
struct OxahostBillingProviderTests {
    ...
    func sumsInvoices() async throws {
        ...
        let client = LiveProviderHarness.stub([
            (
                OxahostBillingProvider.invoicesURL,
        ...
        await LiveProviderHarness.expectStatus(401, code: .unauthorized, key: .invalidCredentials) { status in
            try await provider(
                LiveProviderHarness.stub([
                    (OxahostBillingProvider.invoicesURL, LiveProviderHarness.emptyJSON(status: status)),
                ])
    ...
    private var credential: Credential {
        Credential(providerID: .oxahost, fields: [.email: user, .clientSecret: password])
    }

    private func provider(_ client: any HTTPClient) -> OxahostBillingProvider {
        OxahostBillingProvider(
            httpClient: client,
            now: { now },
            calendar: calendar,
