...
struct HostcircleBillingProviderTests {
    ...
    func sumsInvoices() async throws {
        ...
        let client = LiveProviderHarness.stub([
            (
                HostcircleBillingProvider.invoicesURL,
        ...
        await LiveProviderHarness.expectStatus(401, code: .unauthorized, key: .invalidCredentials) { status in
            try await provider(
                LiveProviderHarness.stub([
                    (HostcircleBillingProvider.invoicesURL, LiveProviderHarness.emptyJSON(status: status)),
                ])
    ...
    private var credential: Credential {
        Credential(providerID: .hostcircle, fields: [.email: user, .clientSecret: password])
    }

    private func provider(_ client: any HTTPClient) -> HostcircleBillingProvider {
        HostcircleBillingProvider(
            httpClient: client,
            now: { now },
            calendar: calendar,
