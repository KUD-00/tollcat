...
App Store のバイナリは Apple が再署名・暗号化するため、このソースからバイト単位で同じものは作れません。確かめられるのは：公開 CI がタグから IPA をビルドして署名付きの来歴を添付すること、そして Mach-O の `LC_UUID` でそのビルドと手元のアプリを突き合わせることです。手順とコマンド、できないことは [VERIFY.md](VERIFY.md)（英語）にあります。
